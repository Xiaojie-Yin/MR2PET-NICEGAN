...
class NICE(object) :
    ...
    def build_model(self):
        ...
        train_transform = A.Compose([
            ...
            A.Affine(scale=(0.85, 1.15), rotate=(-8, 8), shear=(-8, 8)),  # 随机仿射变换
            A.ElasticTransform(alpha=1, sigma=25, alpha_affine=25),  # 随机弹性变形
            A.GaussNoise(var_limit=(0, 0.25*255)),  # 添加随机噪声
            A.RandomGamma(gamma_limit=(70, 130)),  # 随机Gamma变换
        ...
        # 使用 ImageDataset 加载数据
        self.trainA = ImageDataset('/workspace/dataset/csv_new/cloud_174_train_half.csv', train_transform, None)
        self.trainB = ImageDataset('/workspace/dataset/csv_new/cloud_174_train_half.csv', None, train_transform)
        self.testA = ImageDataset('/workspace/dataset/csv_new/cloud_174_test_half.csv', test_transform, None)
        self.testB = ImageDataset('/workspace/dataset/csv_new/cloud_174_test_half.csv', None, test_transform)
    ...
    def train(self):
        ...
        # 确保loss文件夹存在
        loss_dir = os.path.join(self.result_dir, 'loss')
        os.makedirs(loss_dir, exist_ok=True)
        # 定义CSV文件路径
        csv_path = os.path.join(loss_dir, 'loss_data.csv')
        
        # 检查文件是否已存在，如果不存在则创建并写入表头
        if not os.path.exists(csv_path):
            pd.DataFrame(columns=['Step', 'D_Loss', 'G_Loss']).to_csv(csv_path, index=False)
        ...
        trainA_iter = iter(self.trainA_loader) # 加上的
        trainB_iter = iter(self.trainB_loader) # 加上的
        testA_iter = iter(self.testA_loader) # 加上的
        testB_iter = iter(self.testB_loader) # 加上的
        ...
        for step in range(self.start_iter, self.iteration + 1):
            ...
            # 获取 real_A
            try:
                real_A_dict = next(trainA_iter)
            except StopIteration:  # 正确的异常类型
                trainA_iter = iter(self.trainA_loader)
                real_A_dict = next(trainA_iter)

            real_A = real_A_dict['A'].to(self.device)  # 确保从字典中正确提取并转移到设备

            # 获取 real_B
            try:
                real_B_dict = next(trainB_iter)
            except StopIteration:  # 正确的异常类型
                trainB_iter = iter(self.trainB_loader)
                real_B_dict = next(trainB_iter)

            real_B = real_B_dict['B'].to(self.device)  # 确保从字典中正确提取并转移到设备


            # Update D
            self.D_optim.zero_grad()

            # print("Before calling disA, shape of real_A:", real_A.shape)
            ...
            # 每300步保存当前步的损失信息到同一个CSV文件中
            if step % 300 == 0 or step == self.start_iter:
                # 创建一个临时DataFrame来保存当前损失
                current_loss_data = pd.DataFrame({
                    'Step': [step],
                    'D_Loss': [Discriminator_loss.item()],
                    'G_Loss': [Generator_loss.item()]
                })
                
                # 保存DataFrame到CSV文件中，追加模式
                current_loss_data.to_csv(csv_path, mode='a', header=False, index=False)
                print(f"Loss data appended to '{csv_path}' at step {step}")

            # 计算已过时间和预估的剩余时间
            elapsed_time = time.time() - start_time
            remaining_time = elapsed_time * (self.iteration - step) / (step + 1)
            ...
            print("[%5d/%5d] time: %4.4f d_loss: %.8f, g_loss: %.8f, elapsed: %02d:%02d:%02d, remaining: %02d:%02d:%02d" % (
                step, self.iteration, time.time() - start_time, Discriminator_loss, Generator_loss, elapsed_hours,
                elapsed_minutes, elapsed_seconds, hours,
                minutes, seconds))
            ...
            if step % self.print_freq == 0:
                ...
                for _ in range(train_sample_num):
                    ...
                    # 确保real_A_dict和real_B_dict已正确提取，并转移到CPU上
                    if isinstance(real_A_dict, dict) and 'A' in real_A_dict and isinstance(real_B_dict, dict) and 'B' in real_B_dict:
                        real_A = real_A_dict['A'].to('cpu')
                        real_B = real_B_dict['B'].to('cpu')
                    else:
                        print("Error: Data from DataLoader is not in expected format.")
                ...
                for _ in range(test_sample_num):
                    try:
                        real_A_dict = next(testA_iter)
                    except StopIteration:
                        testA_iter = iter(self.testA_loader)
                        real_A_dict = next(testA_iter)

                    try:
                        real_B_dict = next(testB_iter)
                    except StopIteration:
                        testB_iter = iter(self.testB_loader)
                        real_B_dict = next(testB_iter)

...
class ResnetGenerator(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, n_blocks=6, img_size=256, light=False): #原来是256
        assert(n_blocks >= 0)
...
class Discriminator_old(nn.Module):
    ...
    def forward(self, input):
        ...
        out0 = self.conv0(x0)
        out1 = self.conv1(x1)
        print("Shape after conv0:", out0.shape)
        print("Shape after conv1:", out1.shape)
        print("Shape of cam_logit:", cam_logit.shape)
...
class ChannelReducer(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.conv1x1 = nn.Conv2d(in_channels, out_channels, kernel_size=1)  # 使用1x1卷积进行通道数降维

    def forward(self, x):
        return self.conv1x1(x)  # 将输入的特征图从in_channels降维到out_channels

class Discriminator(nn.Module):
    def __init__(self, input_nc, ndf=64, n_layers=7):
        super(Discriminator, self).__init__()
        self.input_nc = input_nc + 1  # Including gradient channel
        
        # Initial convolution layer
        self.initial = nn.Sequential(
            nn.ReflectionPad2d(1),
            nn.utils.spectral_norm(
                nn.Conv2d(self.input_nc, ndf * 2, kernel_size=4, stride=2, padding=0, bias=True)),
            nn.LeakyReLU(0.2, True)
        )
        
        # Middle layers before the branches start
        self.middle = nn.ModuleList()
        for i in range(1, n_layers - 5):
            mult = 2 ** i
            self.middle.append(nn.Sequential(
                nn.ReflectionPad2d(1),
                nn.utils.spectral_norm(
                    nn.Conv2d(ndf * mult, ndf * mult * 2, kernel_size=4, stride=2, padding=0, bias=True)),
                nn.LeakyReLU(0.2, True)
        ...
        # Branch Dis0_0 and Dis0_1
        mult = 2 ** (n_layers - 5 - 1)
        self.Dis0_0 = nn.Sequential(
            nn.ReflectionPad2d(1),
            nn.utils.spectral_norm(
                nn.Conv2d(ndf * mult * 2, ndf * mult * 2, kernel_size=4, stride=2, padding=0, bias=True)),
            nn.LeakyReLU(0.2, True)
        )

        self.Dis0_1 = nn.Sequential(
            nn.ReflectionPad2d(1),
            nn.utils.spectral_norm(
                nn.Conv2d(ndf * mult * 2, ndf * mult * 4, kernel_size=4, stride=1, padding=0, bias=True)),
            nn.LeakyReLU(0.2, True)
        )
        
        mult = 2 ** (n_layers - 5)
        self.fc = nn.utils.spectral_norm(nn.Linear(ndf * mult * 8, 1, bias=False)) # 加
        self.pad = nn.ReflectionPad2d(1) # 加
        self.conv0 = nn.utils.spectral_norm(
            nn.Conv2d(ndf * mult * 2, 1, kernel_size=4, stride=1, padding=0, bias=False)
        )

        # Branch Dis1_0 and Dis1_1
        self.Dis1_0 = nn.ModuleList()
        for i in range(n_layers - 5, n_layers - 3):
            mult = 2 ** i
            self.Dis1_0.append(nn.Sequential(
                nn.ReflectionPad2d(1),
                nn.utils.spectral_norm(
                    nn.Conv2d(ndf * mult, ndf * mult * 2, kernel_size=4, stride=2, padding=0, bias=True)),
                nn.LeakyReLU(0.2, True)
        ...
        mult = 2 ** (n_layers - 3 - 1)
        self.Dis1_1 = nn.Sequential(
            nn.ReflectionPad2d(1),
            nn.utils.spectral_norm(
                nn.Conv2d(ndf * mult * 2, ndf * mult * 2, kernel_size=4, stride=1, padding=0, bias=True)),
            nn.LeakyReLU(0.2, True)
        )
        ...
        # 在Discriminator类的构造器中创建ChannelReducer实例
        self.reducer = ChannelReducer(256, 128)
    ...
    def compute_gradients(self, x):
        ...
        if torch.isnan(gradients).any():
            print("Warning: NaN values detected in gradient. Replacing NaNs with zero.")
            gradients = torch.nan_to_num(gradients, nan=0.0)  # Replace NaN with zero

        return gradients

    def plot_images(self, original_img, gradient_img):
        original_img = original_img.detach().cpu().squeeze()[0].numpy()  # 假设batch size至少为1
        gradient_img = gradient_img.detach().cpu().squeeze()[0].numpy()
        

    def forward(self, input):
        # 确保将reducer移动到正确的设备
        if input.is_cuda:
            self.reducer.to(input.device)

        gradient = self.compute_gradients(input)
        input_with_grad = torch.cat([input, gradient], dim=1)
        
        x = self.initial(input_with_grad)
        ...
        gap = torch.nn.functional.adaptive_avg_pool2d(x1, 1)
        gmp = torch.nn.functional.adaptive_max_pool2d(x1, 1)
        cam_logit = torch.cat([gap, gmp], 1)
        cam_logit = self.fc(cam_logit.view(cam_logit.shape[0], -1))
        heatmap = torch.sum(x, dim=1, keepdim=True)  # Sum along channels to simulate a heatmap

        z = x  # The final feature map passed to further layers
        z_reduced = self.reducer(z)  # 应用降维

        return out0, out1, cam_logit, heatmap, z_reduced